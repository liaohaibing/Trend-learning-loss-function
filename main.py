...
X_train_input=np.loadtxt("data/train_input.txt",delimiter=",") # 读入的时候也需要指定逗号分隔
X_train_target=np.loadtxt("data/train_target.txt",delimiter=",") # 读入的时候也需要指定逗号分隔
train_bkp=np.loadtxt("data/train_bkp.txt",delimiter=",") # 读入的时候也需要指定逗号分隔
X_test_input=np.loadtxt("data/test_input.txt",delimiter=",") # 读入的时候也需要指定逗号分隔
X_test_target=np.loadtxt("data/test_target.txt",delimiter=",") # 读入的时候也需要指定逗号分隔
...
net_gru_tre = Net_GRU(encoder,decoder, N_output, device).to(device)
train_model(net_gru_tre,loss_type='tre',learning_rate=0.001, epochs=500, gamma=gamma, print_every=50, eval_every=50,verbose=1)
...
nets = [net_gru_mse,net_gru_dilate,net_gru_tre]

for ind in range(1,51):
    plt.figure()
    plt.rcParams['figure.figsize'] = (17.0,5.0)
    pred = net_gru_dilate(test_inputs).to(device)
